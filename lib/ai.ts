...
export async function analyzeReport(
  ...
  // Load knowledge context (user memory, glossary, area briefing) — non-blocking best-effort
  let knowledgePrefix = ''
  try {
  ...
  } catch { /* knowledge injection is best-effort — analysis continues without it */ }
...
/** Load userMemory + glossary terms + area briefing for a given area and inject-ready blocks. */
export async function loadKnowledgeForArea(area: string): Promise<KnowledgeContext> {
  // Seed glossary on first call if table is empty (lazy, non-blocking on errors)
  try {
  ...
  // Format glossary as compact inline blocks per scope
  const byScope: Record<string, string[]> = {}
...
/** Auto-generate (or refresh) an area briefing from recent reports. Upserts to DB. */
