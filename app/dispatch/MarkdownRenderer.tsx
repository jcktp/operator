...
export function renderMarkdown(text: string, onDownload: (code: string, lang: string) => void) {
 ...
 return (
 <div key={i} className="my-2 rounded-[10px] overflow-hidden border border-[var(--border)]">
 <div className="flex items-center justify-between bg-[var(--ink)] px-3 py-1.5">
 <span className="text-[10px] text-[var(--text-muted)] font-mono">{lang}</span>
 <button
 ...
 <pre className="bg-[var(--ink)] text-green-400 text-xs px-3 py-2 overflow-x-auto font-mono whitespace-pre">
 {code}
 </pre>
 </div>
 )
