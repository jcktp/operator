...
const SOURCE_COLOR: Record<string, string> = {
  interview:   'bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300',
  survey:      'bg-violet-50 text-violet-700 dark:bg-violet-950 dark:text-violet-300',
  focus_group: 'bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300',
  document:    'bg-gray-100 text-gray-600 dark:bg-zinc-800 dark:text-zinc-300',
}
  ...
  return (
    <div className="max-w-3xl space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-zinc-50">Quote Bank</h1>
          <p className="text-sm text-gray-400 dark:text-zinc-500 mt-0.5">{quotes.length} verbatim quote{quotes.length !== 1 ? 's' : ''} collected across your research.</p>
        </div>
      ...
      <div className="flex gap-2 flex-wrap items-center">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search quotes…"
          className="border border-gray-200 dark:border-zinc-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 dark:bg-zinc-800 dark:text-zinc-100 w-48" />
      ...
      {showForm && (
        <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl p-5 space-y-3">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-zinc-50">Add Quote</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-zinc-300 mb-1">Speaker</label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-zinc-300 mb-1">Tags (comma separated)</label>
      ...
      {visible.length === 0 ? (
        <div className="text-center py-12 text-gray-400 dark:text-zinc-500 text-sm">No quotes found.</div>
      ) : (
        <div className="space-y-3">
          {visible.map(q => (
            <div key={q.id} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl p-4">
              <div className="flex gap-3">
                <Quote size={16} className="shrink-0 mt-1 text-indigo-400 dark:text-indigo-500" />
                <div className="flex-1 min-w-0 space-y-2">
                  <p className="text-sm text-gray-800 dark:text-zinc-100 leading-relaxed italic">&ldquo;{q.text}&rdquo;</p>
                  <div className="flex items-center gap-2 flex-wrap">
                    {q.speaker && <span className="text-[11px] font-medium text-gray-600 dark:text-zinc-300">— {q.speaker}</span>}
                    {q.context && <span className="text-[11px] text-gray-400 dark:text-zinc-500">{q.context}</span>}
                    <span className={cn('text-[10px] px-1.5 py-0.5 rounded font-medium', SOURCE_COLOR[q.sourceType])}>
                      {SOURCE_LABEL[q.sourceType]}
                    </span>
                    {q.tags.map(tag => (
                      <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-zinc-400">
                        {tag}
                      </span>
                    ))}
                  </div>
                </div>
                <button onClick={() => void handleDelete(q.id)} className="text-gray-300 dark:text-zinc-600 hover:text-red-400 transition-colors shrink-0">
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
