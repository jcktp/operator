...
export default function ExportButton({ rows, period }: { rows: ExportRow[]; period: string }) {
 ...
 <button
 onClick={download}
 className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-[var(--text-subtle)] bg-[var(--surface)] border border-[var(--border)] rounded-[4px] hover:border-[var(--border)] hover:text-[var(--text-bright)] transition-colors"
 >
 <Download size={13} />
 Export CSV
 </button>
