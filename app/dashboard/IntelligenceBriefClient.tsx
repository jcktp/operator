...
function ContextNotesSidebar() {
 ...
 return (
 <aside className="w-72 shrink-0 h-full flex flex-col bg-[var(--surface)] border border-[var(--border)] rounded-[10px] overflow-hidden shadow-sm">
 {/* Header */}
 <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)] shrink-0">
 <span className="text-sm font-semibold text-[var(--text-bright)]">Context & Notes</span>
 {hasSelection && (
 <button onClick={close} className="p-1 text-[var(--text-muted)] hover:text-[var(--text-subtle)] rounded transition-colors">
 <X size={13} />
 </button>
 )}
 </div>
 ...
 rows={2}
 className="w-full text-xs border border-[var(--border)] rounded-[10px] px-3 py-2 resize-none focus:outline-none focus:ring-2 bg-[var(--surface)] text-[var(--text-body)] placeholder-gray-300 overflow-y-auto max-h-48"
 ...
 <ScanSearch size={16} className="text-[var(--border)]" />
 <p className="text-[10px] text-[var(--text-muted)]">Dynamically populates based on selected entities or timeline events on the left.</p>
 </div>
 )}
 {hasSelection && selected.type === 'entity' && (
 <>
 <div className="flex items-center gap-1.5 mb-2">
 <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium border ${ENTITY_COLORS[selected.entityType] ?? 'bg-[var(--surface-2)] text-[var(--text-subtle)] border-[var(--border)]'}`}>
 ...
 <span className="shrink-0 text-[10px] px-1.5 py-0.5 rounded bg-[var(--surface-2)] text-[var(--text-muted)] mt-0.5">{a.area}</span>
 <p className="text-xs text-[var(--text-body)] group-hover:text-[var(--blue)] truncate">{a.reportTitle}</p>
 </Link>
 ))}
 </>
 )}
 ...
 <Link key={id} href={`/reports/${id}`} className="block text-xs text-[var(--text-body)] hover:text-[var(--blue)] truncate py-0.5">
 ...
 >
 <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium border ${ENTITY_COLORS[e.entityType] ?? 'bg-[var(--surface-2)] text-[var(--text-subtle)] border-[var(--border)]'}`}>
 {ENTITY_LABEL[e.entityType] ?? e.entityType}
 </span>
 <span className="text-xs text-[var(--text-subtle)] group-hover:text-[var(--blue)]">{e.name}</span>
 </button>
 ))}
 </div>
 </section>
...
function BriefAskBar({ projectId }: { projectId?: string | null }) {
 ...
 {showHint
 ? <Wand2 size={14} className="absolute left-3 text-indigo-400 pointer-events-none" />
 ...
 className="w-full pl-9 pr-9 py-2 text-sm border border-[var(--border)] rounded-[10px] bg-[var(--surface)] text-[var(--text-bright)] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition-shadow"
 />
 {query && (
 <button type="button" onClick={clear} className="absolute right-3 text-[var(--text-muted)] hover:text-[var(--text-subtle)]">
 <X size={13} />
 </button>
 )}
 </div>
 {showHint && (
 <p className="text-[10px] text-[var(--blue)] mt-1 ml-1">Press Enter to ask AI</p>
 )}
 </form>
 {(loading || answer || error) && (
 <div className="mt-2 rounded-[10px] border border-indigo-100/60 bg-indigo-50/60/30 p-3 space-y-2">
 {loading && (
 <div className="flex items-center gap-2 text-xs text-[var(--blue)]">
 ...
 {answer && (
 <>
 <p className="text-sm text-[var(--text-body)] leading-relaxed">{answer.text}</p>
 ...
 key={s.id}
 href={`/reports/${s.id}`}
 className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-[4px] bg-[var(--surface)] border border-[var(--blue)] text-[var(--blue)] hover:bg-[var(--blue-dim)] transition-colors"
 >
 <FileText size={10} />
...
export default function IntelligenceBriefClient({
 ...
 <h2 className="text-sm font-semibold text-[var(--text-bright)]">Entities</h2>
 ...
 <span className="text-[var(--text-muted)]">{ENTITY_ICONS[type]}</span>
 <span className="text-xs font-medium text-[var(--text-muted)]">{ENTITY_LABEL[type] ?? type}</span>
 <span className="text-xl font-semibold text-[var(--text-bright)] ml-1">{entities.length}</span>
 </div>
 ...
 className="text-[11px] px-2 py-0.5 rounded-[4px] bg-[var(--surface-2)] text-[var(--text-subtle)] hover:bg-[var(--blue-dim)] hover:text-indigo-700 transition-colors"
 >
 {e.name}
 </button>
 ))}
 ...
 <h2 className="text-sm font-semibold text-[var(--text-bright)] flex items-center gap-1.5">
 ...
 <span className="text-[10px] text-[var(--text-muted)]">{formatRelativeDate(new Date(s.updatedAt))}</span>
 {s.unverified > 0 && (
 ...
 <div>
 <div className="flex items-center justify-between mb-3">
 <h2 className="text-sm font-semibold text-[var(--text-bright)]">Timeline</h2>
 ...
 >
 <span className="absolute left-0 top-[8px] -translate-x-1/2 w-2.5 h-2.5 rounded-full border-2 bg-[var(--surface)] border-[var(--border-mid)] transition-all duration-300 group-hover:w-3 group-hover:h-3 group-hover:bg-red-500 group-hover:border-red-500"
 ...
 <span className="text-[11px] font-mono text-[var(--text-muted)]">{ev.dateText}</span>
 <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--surface-2)] text-[var(--text-muted)]">{ev.report.area}</span>
 </div>
 <p className="text-sm text-[var(--text-body)] leading-snug group-hover:text-indigo-700 transition-colors">{ev.event}</p>
 <Link
 href={`/reports/${ev.report.id}`}
 onClick={e => e.stopPropagation()}
 className="text-xs text-[var(--text-muted)] hover:text-[var(--blue)] hover:underline mt-0.5 block"
 >
 {ev.report.title}
 </Link>
 </button>
