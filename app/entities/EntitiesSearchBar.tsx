...
export default function EntitiesSearchBar({
 ...
 className="w-full pl-9 pr-9 py-2 text-sm border border-[var(--border)] rounded-[10px] bg-[var(--surface)] text-[var(--text-bright)] placeholder-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--blue)] transition-shadow"
 />
 {query && (
 <button type="button"onClick={clear} className="absolute right-3 text-[var(--text-muted)] hover:text-[var(--text-body)]">
 <X size={13} />
 </button>
 )}
 </div>
 {showAiHint && (
 <p className="text-[10px] text-[var(--blue)] mt-1 ml-1">
 ...
 <div className="mt-2 rounded-[10px] border border-[var(--blue-dim)] bg-[var(--blue-dim)] p-3 space-y-2">
 {loading && (
 ...
 key={s.id}
 href={`/reports/${s.id}`}
 className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-[4px] bg-[var(--surface)] border border-[var(--blue-dim)] text-[var(--blue)] hover:bg-[var(--surface-2)] transition-colors"
 >
 <FileText size={10} />
