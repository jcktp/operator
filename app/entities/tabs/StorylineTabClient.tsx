...
export default function StorylineTabClient({ stories: initialStories, allReports, allDirects }: Props) {
 ...
 className="w-full text-xs border border-[var(--border)] rounded-[4px] px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-[var(--ink)] bg-[var(--surface)] "
 />
 <button
 ...
 {claimCount > 0 && (
 <span className={`text-[10px] shrink-0 ${isActive ? 'text-[var(--text-muted)]' : 'text-[var(--text-muted)]'}`}>
 {claimCount}
 </span>
 )}
 ...
 <BookOpen size={24} className="text-[var(--text-muted)] mb-3" />
 <p className="text-sm text-[var(--text-subtle)]">Select a story or create one to get started.</p>
 </div>
 )}
 </div>
 </div>
