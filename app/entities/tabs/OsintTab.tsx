...
export default function OsintTab() {
 ...
 className="w-full pl-8 pr-8 py-2 text-sm border border-[var(--border)] rounded-[4px] bg-[var(--surface)] text-[var(--text-bright)] placeholder-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--ink)]"
 />
 {query && (
 <button
 onClick={() => setQuery('')}
 className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[var(--text-muted)] hover:text-[var(--text-body)]"
 >
 <X size={13} />
 </button>
 )}
