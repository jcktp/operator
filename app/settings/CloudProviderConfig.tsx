...
export default function CloudProviderConfig({
 ...
 <label className="block text-xs font-medium text-[var(--text-subtle)]">API Key</label>
 <a href={p.docsUrl} target="_blank" rel="noopener noreferrer"
 className="text-xs text-[var(--text-muted)] hover:text-[var(--text-subtle)] flex items-center gap-1">
 ...
 className="flex-1 h-8 border border-[var(--border)] rounded-[4px] px-2.5 text-xs font-mono focus:outline-none focus:ring-2"
 />
 <button
 type="button"
 ...
 className="shrink-0 border border-[var(--border)] text-[var(--text-subtle)] text-xs font-medium px-3 py-2 rounded-[4px] hover:bg-[var(--surface-2)] transition-colors disabled:opacity-40"
 >
 ...
 <p className="text-xs text-[var(--green)] mt-1.5 flex items-center gap-1">
 ...
 <div>
 <label className="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Model</label>
 <select
 value={selectedModels[activeProvider]}
 onChange={e => setSelectedModels(m => ({ ...m, [activeProvider]: e.target.value }))}
 className="w-full h-8 border border-[var(--border)] rounded-[4px] px-2.5 text-xs font-mono text-[var(--text-bright)] focus:outline-none focus:ring-2 bg-[var(--surface)]"
 >
 ...
 <div className="bg-[var(--surface-2)] rounded-[4px] p-3 space-y-1.5">
 <p className="text-xs font-medium text-[var(--text-subtle)]">About this provider</p>
 <p className="text-xs text-[var(--text-muted)]">{p.note}</p>
 <p className="text-xs text-[var(--text-muted)]">Model used: <code className="font-mono">{p.model}</code></p>
 </div>

 {savedProvider === 'ollama' && savedModel && (
 <div className="bg-[var(--amber-dim)] border border-amber-200 rounded-[4px] px-3 py-2.5 text-xs text-[var(--amber)] flex items-start gap-2">
