...
export default async function OnePagerPage() {
 ...
 <p className="text-[var(--text-muted)] text-sm">No {modeConfig.documentLabelPlural.toLowerCase()} yet.</p>
 </div>
 )
 }
 ...
 <p className="text-[var(--text-muted)] text-sm mt-0.5">All {reports.length} {modeConfig.documentLabelPlural.toLowerCase()} in one view</p>
 </div>
 ...
 m.status === 'warning' ? 'text-amber-700' : 'text-[var(--text-bright)]'
 }`}>{m.value}</p>
 </div>
 ))}
 </div>
 ...
 <div className="mb-3">
 <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-2">Flags</p>
 ...
 <div>
 <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-2">Key questions</p>
 <div className="space-y-1">
