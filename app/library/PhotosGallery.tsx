...
export default function PhotosGallery({ photos: initial }: Props) {
 ...
 <ImageOff size={24} className="text-[var(--border)]" />
 <p className="text-sm text-[var(--text-muted)]">No images in this area yet.</p>
 </div>
 )
 }
 ...
 <div
 key={photo.id}
 className="bg-[var(--surface)] border border-[var(--border)] rounded-[10px] hover:border-[var(--border-mid)] hover:shadow-sm transition-all flex gap-4 p-3 items-start group"
 >
 {/* Thumbnail */}
 <button
 ...
 className="text-sm font-semibold text-[var(--text-bright)] hover:underline truncate"
 >
 {photo.title}
 </Link>
 </div>
 ...
 {photo.storyName && (
 <span className="text-[var(--blue)] font-medium">{photo.storyName}</span>
 )}
 </div>

 {/* Show the vision/OCR description, not the report summary */}
 {photo.rawContent && !photo.rawContent.startsWith('[Image') && (
 <p className="text-sm text-[var(--text-subtle)] line-clamp-4 leading-relaxed">
 {photo.rawContent}
 </p>
 )}
 ...
 <Link
 href={`/reports/${photo.id}`}
 className="inline-block mt-2 text-xs text-[var(--text-muted)] hover:text-[var(--text-body)] transition-colors"
 >
 View full analysis →
 </Link>
 </div>
 ...
 alt={lightbox.title}
 className="max-w-full max-h-[78vh] object-contain mx-auto rounded-[10px] block"
 />
 ...
 <button onClick={close} className="p-1.5 rounded-[4px] bg-[var(--surface)]/10 hover:bg-[var(--surface)]/20 text-white transition-colors">
 <X size={14} />
 </button>
 </div>
 </div>
