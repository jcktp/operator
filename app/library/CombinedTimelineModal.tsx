...
export default function CombinedTimelineModal({
 ...
 <div
 className="bg-[var(--surface)] rounded-[10px] shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col"
 onClick={e => e.stopPropagation()}
 >
 {/* Header */}
 <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
 <h2 className="text-base font-semibold text-[var(--text-bright)] flex items-center gap-2">
 <Clock size={16} />
 Combined Timeline
 ...
 <button
 onClick={onClose}
 className="text-[var(--text-muted)] hover:text-[var(--text-subtle)] transition-colors"
 >
 <X size={18} />
 </button>
 </div>
 ...
 <div className="w-2 h-2 rounded-full bg-[var(--surface-3)] shrink-0" />
 {i < events.length - 1 && (
 <div className="w-px bg-[var(--surface-2)] flex-1 min-h-[20px] mt-1" />
 )}
 </div>
 <div className="flex-1 min-w-0 pb-4">
 <div className="flex items-baseline gap-2 flex-wrap mb-0.5">
 <span className="text-xs font-semibold text-[var(--text-muted)]">{event.dateText}</span>
 <Link
 href={`/reports/${event.reportId}`}
 className={`text-xs px-1.5 py-0.5 rounded font-medium hover:opacity-80 transition-opacity ${reportColorMap.get(event.reportId) ?? 'bg-[var(--surface-2)] text-[var(--text-subtle)]'}`}
 >
