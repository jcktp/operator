      ...
      <div className="max-w-2xl mx-auto px-6 pt-16 pb-12">
        <div className="mb-12">
          <div className="flex items-center gap-2 mb-3">
            <Globe size={20} className="text-gray-400 dark:text-zinc-500" />
            <h1 className="text-lg font-semibold text-gray-700 dark:text-zinc-300">Operator Browser</h1>
          </div>
        ...
        <div className="mb-10">
          <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-4 dark:text-zinc-500">What you can do here</p>
          <div className="grid grid-cols-2 gap-3">
            {features.map(f => (
              <div key={f.title} className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm dark:bg-zinc-900 dark:border-zinc-800">
                <div className="flex items-center gap-2 mb-2">
                  {f.icon}
                  <span className="text-sm font-medium text-gray-800 dark:text-zinc-100">{f.title}</span>
                </div>
                <p className="text-xs text-gray-500 leading-relaxed dark:text-zinc-400">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {bookmarks.length > 0 && (
          <div>
            <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-3 dark:text-zinc-500">Bookmarks</p>
            <ul className="space-y-1">
              {bookmarks.map(bm => (
                <li key={bm.id}>
                  <button
                    onClick={() => onNavigate(bm.url)}
                    className="w-full flex items-center gap-2.5 px-3 py-2.5 bg-white rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-sm text-left transition-all dark:bg-zinc-900 dark:border-zinc-800 dark:hover:border-zinc-700"
                  >
                    {bm.favicon ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={bm.favicon} alt="" className="w-4 h-4 shrink-0" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
                    ) : (
                      <Globe size={13} className="text-gray-400 shrink-0 dark:text-zinc-500" />
                    )}
                    <span className="flex-1 text-xs text-gray-700 truncate font-medium dark:text-zinc-200">{bm.title}</span>
                    <span className="text-[10px] text-gray-400 truncate max-w-[120px] dark:text-zinc-500">{bm.url.replace(/^https?:\/\//, '').split('/')[0]}</span>
                  </button>
                </li>
              ))}
            </ul>
