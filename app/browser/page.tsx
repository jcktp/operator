...
import { ArrowLeft, ArrowRight, X, Bookmark, BookmarkCheck, ExternalLink, Loader2, AlignLeft, Monitor, Plus } from 'lucide-react'
import { BookOpen } from 'lucide-react'
import { cn } from '@/lib/utils'
import DispatchPanel from '@/app/dispatch/DispatchPanel'
import { isYouTubeVideo, isYouTubeDomain, isSpotifyEmbed, isEmbedUrl, tabLabel } from './browserHelpers'
...
export default function BrowserPage() {
  ...
  const isBookmarked = b.bookmarks.some(bm => bm.url === activeTab.currentUrl)
    ...
    <div className="flex h-full">
      <div className="flex-1 flex flex-col min-w-0 h-full">
        ...
        <div className="flex items-center gap-1 bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-700 px-3 py-1.5 overflow-x-auto shrink-0">
          {b.tabs.map(tab => {
            const isActive = tab.id === b.activeTabId
            const label = tabLabel(tab)
            const favicon = tab.page?.type === 'article' ? tab.page.favicon : null
            return (
              <div
                key={tab.id}
                onClick={() => b.setActiveTabId(tab.id)}
                className={cn(
              ...
              >
                ...
                <button
                  onClick={e => { e.stopPropagation(); b.closeTab(tab.id) }}
          ...
          <button onClick={b.addTab} title="New tab" className="flex items-center justify-center w-6 h-6 ml-0.5 rounded-md text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors shrink-0">
            <Plus size={12} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shrink-0">
          <button onClick={b.goBack} disabled={!canBack} className={cn('p-1.5 rounded-md transition-colors', canBack ? 'text-gray-600 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800' : 'text-gray-300 dark:text-zinc-600 cursor-default')}>
            <ArrowLeft size={15} />
          </button>
          <button onClick={b.goForward} disabled={!canForward} className={cn('p-1.5 rounded-md transition-colors', canForward ? 'text-gray-600 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800' : 'text-gray-300 dark:text-zinc-600 cursor-default')}>
            <ArrowRight size={15} />
          </button>

          <form onSubmit={b.handleSubmit} className="flex-1">
            <input
              type="text"
              value={activeTab.urlInput}
              onChange={e => b.updateTab(activeTab.id, { urlInput: e.target.value })}
          ...
          <button onClick={b.toggleBookmark} disabled={!activeTab.currentUrl} title={isBookmarked ? 'Remove bookmark' : 'Add bookmark'} className={cn('p-1.5 rounded-md transition-colors', activeTab.currentUrl ? 'text-gray-600 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800' : 'text-gray-300 dark:text-zinc-600 cursor-default')}>
            {isBookmarked ? <BookmarkCheck size={15} className="text-indigo-600" /> : <Bookmark size={15} />}
          </button>

          <div className="relative" ref={b.bookmarkPanelRef}>
            <button onClick={() => b.setShowBookmarks(s => !s)} title="Bookmarks" className={cn('p-1.5 rounded-md transition-colors', b.showBookmarks ? 'bg-gray-100 dark:bg-zinc-800 text-gray-800 dark:text-zinc-100' : 'text-gray-500 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800')}>
              <BookOpen size={15} />
            </button>
            {b.showBookmarks && (
              <div className="absolute left-0 top-full mt-1 w-72 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl shadow-lg z-30 overflow-hidden">
                <div className="px-3 py-2 border-b border-gray-100 dark:border-zinc-800 flex items-center justify-between">
                  ...
                  <button onClick={() => b.setShowBookmarks(false)} className="text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-zinc-300"><X size={12} /></button>
                </div>
                {b.bookmarks.length === 0 ? (
                  ...
                  <ul className="max-h-72 overflow-y-auto">
                    {b.bookmarks.map(bm => (
                      <li key={bm.id}>
                        <button onClick={() => { b.navigateActive(bm.url); b.setShowBookmarks(false) }} className="w-full flex items-center gap-2.5 px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-zinc-800 text-left transition-colors">
                          {bm.favicon ? (
                            // eslint-disable-next-line @next/next/no-img-element
          ...
          {activeTab.currentUrl && (
            <button onClick={b.importToOperator} disabled={b.importing || b.importDone} title={b.importDone ? 'Imported!' : 'Import to Operator'}
              className={cn('flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors shrink-0',
                b.importDone ? 'bg-green-50 dark:bg-green-950 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800' : 'bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 dark:hover:bg-indigo-900')}
            >
              {b.importing ? <Loader2 size={12} className="animate-spin" /> : <Upload size={12} />}
              {b.importDone ? 'Imported' : 'Import'}
            </button>
          )}
        ...
        <div className="flex-1 relative overflow-hidden bg-white dark:bg-zinc-900">
          ...
          {!activeTab.currentUrl && (
            <BrowserStartPage onNavigate={b.navigateActive} bookmarks={b.bookmarks} />
          )}
          ...
          {!activeTab.loading && activeTab.viewMode === 'reader' && activeTab.page?.type === 'article' && (
            <div className="h-full overflow-y-auto bg-[#fafafa] dark:bg-zinc-950 relative" onMouseUp={b.handleTextSelect}>
              ...
              {b.selectedText && (
                <div className="sticky bottom-16 left-0 right-0 flex justify-center pointer-events-none">
                  ...
                  >
                    ...
                    <button onClick={() => b.setSelectedText('')} className="text-gray-400 dark:text-zinc-500 hover:text-white dark:hover:text-zinc-900 transition-colors ml-1">
                      <X size={12} />
                    </button>
                  </div>
                </div>
            ...
            <div className="h-full flex flex-col items-center justify-center gap-5 text-center px-8 bg-[#fafafa] dark:bg-zinc-950">
              ...
              <div className="flex flex-col gap-2 w-full max-w-[220px]">
                ...
                {activeTab.viewMode === 'reader' && (
                  <button onClick={() => b.switchMode('live')} className="flex items-center justify-center gap-2 w-full px-4 py-2 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-gray-700 dark:text-zinc-200 text-xs font-medium rounded-xl transition-colors">
                ...
                onLoad={e => {
                  try {
                    ...
                    if (newUrl && newUrl !== 'about:blank' && newUrl !== activeTab.currentUrl) {
                      b.updateTab(activeTab.id, { urlInput: newUrl, currentUrl: newUrl })
                      const tab = b.tabsRef.current.find(t => t.id === activeTab.id)
                      if (tab) {
                        const newHistory = [...tab.history.slice(0, tab.historyIndex + 1), newUrl]
                        b.updateTab(activeTab.id, { history: newHistory, historyIndex: newHistory.length - 1 })
                      }
                    }
                  } catch { /* cross-origin */ }
                }}
              ...
              <div className="absolute bottom-0 left-0 right-0 z-20 pointer-events-none flex justify-center pb-4">
                <div className="pointer-events-auto flex items-center gap-3 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm border border-gray-200 dark:border-zinc-700 rounded-xl shadow-md px-4 py-2.5">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => b.switchMode('reader')} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-gray-700 dark:text-zinc-200 text-xs font-medium rounded-lg transition-colors">
                      <AlignLeft size={12} /> Try Reader
                    </button>
                    <button onClick={() => openExternal(activeTab.currentUrl)} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-gray-900 dark:bg-zinc-100 hover:bg-gray-700 dark:hover:bg-zinc-200 text-white dark:text-zinc-900 text-xs font-medium rounded-lg transition-colors">
      ...
      <aside className="w-[320px] shrink-0 h-full border-l border-gray-200 dark:border-zinc-700 overflow-hidden">
        <DispatchPanel context={b.dispatchContext} currentUrl={activeTab?.currentUrl ?? ''} />
      </aside>

      {/* Text selection import modal */}
      {b.showImportModal && b.selectedText && (
        <BrowserImportModal
          initialTitle={b.importModalTitle}
          initialDate={b.importModalDate}
          text={b.selectedText}
          onClose={() => b.setShowImportModal(false)}
          onImported={() => {
            b.setShowImportModal(false)
            b.setSelectedText('')
            b.setImportDone(true)
          }}
        />
      )}
    </div>
