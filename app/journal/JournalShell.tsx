...
export default function JournalShell({ entries: initial, projects = [] }: Props) {
 ...
 const sidebar = (
   <div className="flex flex-col h-full">
     <div className="px-3 pt-4 pb-3 space-y-2.5 border-b border-[var(--border)]">
       <SearchInput
         size="md"
         value={search}
       ...
       <button
         onClick={() => openNewNoteForm(selected?.folder ?? DEFAULT_FOLDER)}
         className="w-full flex items-center justify-center gap-1.5 h-8 text-xs font-medium rounded-full bg-[var(--ink)] text-[var(--ink-contrast)] hover:opacity-90 transition-colors"
       >
         <PenLine size={12} />
         New note
       </button>
       ...
       <div className="flex gap-1">
         <input
           ...
           className="flex-1 text-[11px] border border-dashed border-[var(--border-mid)] rounded-[6px] px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-[var(--blue)] placeholder:text-[var(--text-muted)] min-w-0"
         />
         {newFolderName.trim() && (
           <button onClick={createFolder} className="shrink-0 px-2 py-1.5 bg-[var(--ink)] text-[var(--ink-contrast)] text-[11px] rounded-[6px] hover:opacity-90 transition-colors">
             <Plus size={10} />
           </button>
         )}
       </div>
     ...
     {investigationPrompt && (
       <div className="bg-[var(--amber-dim)] border border-[var(--amber)] rounded-[6px] mx-3 mt-3 px-3 py-2.5 space-y-2">
         <div className="flex items-center gap-1.5">
           <BookMarked size={12} className="text-[var(--amber)] shrink-0" />
           <p className="text-xs font-medium text-[var(--amber)]">Set up investigation folders?</p>
         </div>
         <p className="text-[11px] text-[var(--amber)] leading-snug">
           Sub-folders for <strong>{investigationPrompt}</strong>: Sources, Timeline, Claims, Documents, Notes
         </p>
         <div className="flex gap-1.5">
           <button
             onClick={() => createInvestigationTemplate(investigationPrompt)}
             disabled={creatingTemplate}
             className="flex-1 text-[11px] font-medium px-2 py-1 bg-[var(--amber)] text-white rounded hover:opacity-90 transition-colors disabled:opacity-50"
           >
           ...
           <div key={folder} className="mb-1">
             <div className="flex items-center group">
               <button
                 ...
                 className="flex items-center gap-1.5 flex-1 px-3 py-1.5 text-left hover:bg-[var(--surface-2)] transition-colors min-w-0"
               >
                 {isExpanded ? <ChevronDown size={11} className="text-[var(--text-muted)] shrink-0" /> : <ChevronRight size={11} className="text-[var(--text-muted)] shrink-0" />}
                 <FolderOpen size={12} className={projectByFolderName[folder] ? 'text-[var(--blue)] shrink-0' : 'text-[var(--amber)] shrink-0'} />
               ...
               <button
                 ...
                 className="shrink-0 p-1 mr-1 text-[var(--text-muted)] hover:text-[var(--text-body)] hover:bg-[var(--surface-2)] rounded transition-colors"
             ...
             {isExpanded && (
               <div className="ml-4 mr-1 mt-0.5 space-y-0.5">
                 {newNoteFolder === folder && (
                   <div className="flex gap-1 py-1">
                     <input
                       ...
                       className="flex-1 text-xs border border-[var(--border-mid)] rounded-[6px] px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[var(--blue)] min-w-0"
                     />
                     <button
                       onClick={() => createNote()}
                       className="shrink-0 text-[10px] font-medium px-2 py-1 bg-[var(--ink)] text-[var(--ink-contrast)] rounded-[6px] hover:opacity-90 whitespace-nowrap"
                     >
                       Save
                     </button>
                   </div>
                 ...
                 {folderEntries.map(entry => (
                   <div
                     ...
                     className={`flex items-center gap-1.5 px-2 py-1.5 rounded-[6px] cursor-pointer group/note transition-colors border-l-2 ${
                       selectedId === entry.id
                 ...
                 {folderEntries.length === 0 && newNoteFolder !== folder && (
                   <p className="text-[10px] text-[var(--text-muted)] px-2 py-1">Empty</p>
                 )}
               </div>
             )}
           </div>
 ...
 const editorPane = selected ? (
       ...
       <div className="flex items-center gap-1 shrink-0">
         {viewMode === 'view' ? (
           <button
             ...
             className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-full bg-[var(--ink)] text-[var(--ink-contrast)] hover:opacity-90 transition-colors"
           >
             <Edit2 size={11} /> Edit
           </button>
         ) : (
           <button
             ...
             className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-full bg-[var(--surface-2)] text-[var(--text-body)] hover:bg-[var(--surface-3)] transition-colors"
           >
             <Eye size={11} /> Done
           </button>
         )}
   ...
   <div className="flex flex-col items-center justify-center h-full text-center py-20">
     ...
     <button
       ...
       className="inline-flex items-center gap-1.5 text-xs font-medium px-4 py-2 bg-[var(--ink)] text-[var(--ink-contrast)] rounded-full hover:opacity-90 transition-colors"
     >
 ...
 return (
   <LayoutB sidebar={sidebar} sidebarWidth={220} contentMaxWidth={null}>
     {editorPane}
   </LayoutB>
 )
}
