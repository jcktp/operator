...
export default function RequestPage({ params }: { params: Promise<{ token: string }> }) {
 ...
 <Loader2 size={24} className="animate-spin text-[var(--border)]" />
 </div>
 </Shell>
 )
 }
 ...
 <AlertCircle size={32} className="text-[var(--border)] mx-auto mb-3" />
 <p className="text-[var(--text-subtle)] font-medium">Link not found</p>
 <p className="text-sm text-[var(--text-muted)] mt-1">This request link doesn&apos;t exist or has been removed.</p>
 </div>
 </Shell>
 )
 }
 ...
 <p className="text-[var(--text-subtle)] font-medium">Something went wrong</p>
 {errorMsg && <p className="text-xs text-[var(--text-muted)] mt-2 max-w-xs mx-auto">{errorMsg}</p>}
 <button
 onClick={() => { setStage('loading'); setErrorMsg(''); loadRequest(token) }}
 className="mt-4 text-sm text-[var(--text-muted)] underline underline-offset-2"
 >
 Try again
 </button>
 </div>
 ...
 <div className="text-center py-16">
 <AlertCircle size={32} className="text-[var(--border)] mx-auto mb-3" />
 <p className="text-[var(--text-subtle)] font-medium">Link expired</p>
 <p className="text-sm text-[var(--text-muted)] mt-1">This request link is no longer active. Ask for a new one.</p>
 </div>
 </Shell>
 )
 }
 ...
 <p className="text-[var(--text-bright)] font-semibold text-lg">{modeConfig.documentLabel} submitted</p>
 <p className="text-sm text-[var(--text-muted)] mt-1">Thanks — it&apos;s been received and is being reviewed.</p>
 </div>
 </Shell>
 )
 }
 ...
 {/* Context */}
 <div className="mb-8">
 <p className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-1">{modeConfig.documentLabel} request</p>
 <h1 className="text-2xl font-semibold text-[var(--text-bright)]">{info?.title}</h1>
 {info?.directReport && (
 ...
 <div className="mb-5">
 <label className="block text-xs font-medium text-[var(--text-muted)] mb-1.5">
 {modeConfig.documentLabel} date <span className="text-[var(--red)]">*</span>
 </label>
 <input
 type="date"
 value={reportDate}
 ...
 uploadMode === 'file' ? 'bg-[var(--surface)] text-[var(--text-bright)] shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text-body)]'
 )}
 >
 <Upload size={14} />
 Upload file
 ...
 className={cn(
 'flex-1 flex items-center justify-center gap-2 py-2 rounded-md text-sm font-medium transition-colors',
 uploadMode === 'link' ? 'bg-[var(--surface)] text-[var(--text-bright)] shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text-body)]'
 )}
 >
 <Link2 size={14} />
 Google link
 ...
 />
 <Upload size={20} className="text-[var(--border)] mx-auto mb-3" />
 <p className="text-sm font-medium text-[var(--text-subtle)]">Drop your file here or click to browse</p>
 ...
 <button onClick={() => setFile(null)} className="text-xs text-[var(--text-muted)] hover:text-[var(--text-subtle)]">Change</button>
 </div>
 )}
 </div>
 )}
...
function Shell({ children }: { children: React.ReactNode }) {
 ...
 <WalkieTalkie />
 <span className="text-xl text-[var(--text-bright)] "style={{ fontFamily: 'var(--font-caveat)', fontWeight: 700 }}>
 operator
 </span>
 </div>
 </div>
