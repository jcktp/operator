...
function FilePicker({
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-32 text-gray-400 dark:text-zinc-500">
  ...
  if (flat.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-32 text-gray-400 dark:text-zinc-500 gap-1.5 text-sm">
      ...
      {flat.map(node => {
        ...
        return (
          <div
            ...
            className={cn(
              'flex items-center gap-2 py-1.5 pr-3 cursor-pointer text-sm rounded transition-colors',
              isSelected
                ...
                : isDir
                  ...
                  : isCleanable(node.entry.ext)
                    ? 'text-gray-700 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-800'
                    : 'text-gray-400 dark:text-zinc-600 hover:bg-gray-50 dark:hover:bg-zinc-800 cursor-default'
            )}
          >
            {isDir ? (
              <ChevronRight size={12} className={cn('shrink-0 text-gray-400 dark:text-zinc-600 transition-transform', node.expanded && 'rotate-90')} />
            ) : (
              <span className="w-3 shrink-0" />
            )}
            <span className={isDir ? 'text-amber-500' : 'text-gray-400 dark:text-zinc-500'}>
              {isDir
            ...
            {node.childLoading && <Loader2 size={10} className="animate-spin ml-auto shrink-0 text-gray-400" />}
          </div>
        )
      })}
    </div>
...
function MetadataInspector({
  ...
  if (!relativePath) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-gray-400 dark:text-zinc-500 gap-2">
  ...
  if (!metadata) {
    return (
      <div className="flex items-center justify-center h-full text-red-400 dark:text-red-500 text-sm">
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {entries.length === 0 ? (
          <p className="text-xs text-gray-400 dark:text-zinc-500 text-center pt-8">No metadata found</p>
        ) : (
          <table className="w-full text-xs border-collapse">
            <tbody>
              ...
              {sensitive.length > 0 && (
                <>
                  <tr>
                    <td colSpan={2} className="pt-2 pb-1 text-[10px] font-semibold uppercase tracking-wide text-amber-600 dark:text-amber-400">
                      Sensitive
                    </td>
                  </tr>
                  {sensitive.map(([k, v]) => (
                    <tr key={k} className="border-b border-amber-100 dark:border-amber-900/30">
                      <td className="py-1 pr-3 font-medium text-amber-700 dark:text-amber-400 w-2/5 truncate align-top">{k}</td>
                      <td className="py-1 text-amber-600 dark:text-amber-300 break-words">{formatTagValue(v)}</td>
                    </tr>
                  ))}
                </>
              )}
              ...
              {other.length > 0 && (
                <>
                  <tr>
                    <td colSpan={2} className="pt-3 pb-1 text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-zinc-500">
                      All Tags
                    </td>
                  </tr>
                  {other.map(([k, v]) => (
                    <tr key={k} className="border-b border-gray-100 dark:border-zinc-800/50">
                      <td className="py-1 pr-3 font-medium text-gray-600 dark:text-zinc-300 w-2/5 truncate align-top">{k}</td>
                      <td className="py-1 text-gray-500 dark:text-zinc-400 break-words">{formatTagValue(v)}</td>
                    </tr>
                  ))}
                </>
              )}
...
function CleanPanel({
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h3 className="text-xs font-semibold text-gray-700 dark:text-zinc-200 mb-2">Cleaning tool</h3>
        <div className="space-y-2">
          ...
          <label className={cn(
            ...
            />
            <div>
              <div className="text-xs font-medium text-gray-800 dark:text-zinc-100">MAT2 — Strip all metadata</div>
          ...
          <label className={cn(
            ...
            />
            <div>
              <div className="text-xs font-medium text-gray-800 dark:text-zinc-100">ExifTool — Strip EXIF tags</div>
              <div className="text-[11px] text-gray-500 dark:text-zinc-400 mt-0.5">
      ...
      <button
        ...
        className={cn(
          ...
          canClean && !cleaning
            ? 'bg-gray-900 dark:bg-zinc-100 text-white dark:text-zinc-900 hover:bg-gray-700 dark:hover:bg-zinc-300'
      ...
      {/* Success + download */}
      {cleanedPath && (
        <div className="flex flex-col gap-2 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800">
          <div className="flex items-center gap-2">
            <CheckCircle2 size={13} className="shrink-0 text-emerald-600 dark:text-emerald-400" />
            <span className="text-[11px] font-medium text-emerald-700 dark:text-emerald-300">Metadata stripped successfully</span>
          </div>
          <a
            href={`/api/files/download?path=${encodeURIComponent(cleanedPath)}`}
            download
            className="flex items-center justify-center gap-2 w-full py-2 rounded-lg text-xs font-medium bg-emerald-600 hover:bg-emerald-700 text-white transition-colors"
          >
            <Download size={13} />
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="shrink-0 pb-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-gray-900 dark:text-white">File Cleaner</h1>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors dark:text-zinc-400 dark:hover:text-zinc-200 dark:hover:bg-zinc-800"
          >
            <RefreshCw size={12} />
            Refresh
          </button>
      ...
      <div className="flex-1 min-h-0 flex gap-4">
        {/* Left: File picker */}
        <div className="w-[28%] shrink-0 flex flex-col border border-gray-200 dark:border-zinc-700 rounded-lg overflow-hidden">
          <div className="shrink-0 px-3 py-2 border-b border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50">
            <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-zinc-500">Files</span>
          </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col border border-gray-200 dark:border-zinc-700 rounded-lg overflow-hidden">
          <div className="shrink-0 px-3 py-2 border-b border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50">
            <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-zinc-500">Metadata</span>
          </div>
        ...
        <div className="w-[28%] shrink-0 flex flex-col border border-gray-200 dark:border-zinc-700 rounded-lg overflow-hidden">
          <div className="shrink-0 px-3 py-2 border-b border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50">
            <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-zinc-500">Clean</span>
          </div>
