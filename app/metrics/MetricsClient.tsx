  ...
  return (
    <div className="flex gap-6 items-start">
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        {filtered.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-12">No metrics match your filters.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {filtered.map((m, i) => (
              <Link
                ...
                className="group bg-white border border-gray-200 rounded-xl p-4 hover:border-gray-300 hover:shadow-sm transition-all flex flex-col gap-2"
              >
                <div className="flex items-start justify-between gap-2">
                  <span className="text-xs font-medium text-gray-500 leading-snug">{m.label}</span>
                  <ArrowRight size={12} className="text-gray-300 group-hover:text-gray-500 transition-colors shrink-0 mt-0.5" />
                </div>
                ...
                {m.context && (
                  <p className="text-xs text-gray-400 leading-snug">{m.context}</p>
                )}
                <div className="flex items-center gap-1.5 mt-auto pt-1 flex-wrap">
                  <AreaBadge area={m.area} />
                  <span className="text-[11px] text-gray-400 truncate">{m.reportTitle}</span>
                </div>
                {m.sourceName && (
                  <p className="text-[11px] text-gray-400">{m.sourceName} · {formatRelativeDate(new Date(m.reportDate))}</p>
                )}
              </Link>
            ))}
          </div>
