    ...
    <div className="flex flex-col h-full">
      ...
      <div className="shrink-0 pb-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-lg font-semibold text-gray-900 dark:text-white">File Management</h1>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={refresh}
              disabled={refreshing}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors dark:text-zinc-400 dark:hover:text-zinc-200 dark:hover:bg-zinc-800 disabled:opacity-40"
            >
            ...
            <button
              onClick={() => openPath('')}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium bg-gray-900 text-white hover:bg-gray-700 transition-colors dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-300"
            >
              <FolderOpen size={13} />
              Open in Finder
            </button>
      ...
      <div className="shrink-0 pb-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 text-sm rounded-lg border border-gray-200 bg-white placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-300 dark:bg-zinc-900 dark:border-zinc-700 dark:text-zinc-100 dark:placeholder:text-zinc-600 dark:focus:ring-zinc-600"
          />
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-gray-400 dark:text-zinc-600 gap-2">
          ...
          <div>
            {filtered.map(node => {
              ...
              return (
                <div key={node.relativePath} className="border-b border-gray-100 dark:border-zinc-800/60 last:border-0">
                  <div
                    onClick={() => isDir ? toggleFolder(node.relativePath) : openPath(node.relativePath)}
                    style={{ paddingLeft: `${node.depth * 20 + 4}px` }}
                    className="w-full flex items-center gap-2.5 py-2 pr-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800/50 rounded transition-colors group"
                  >
                    {/* Chevron */}
                    {isDir ? (
                      <ChevronRight
                        size={13}
                        className={`shrink-0 text-gray-300 dark:text-zinc-600 transition-transform ${node.expanded ? 'rotate-90' : ''}`}
                      />
                    ) : (
                    ...
                    <span className={
                      isDir
                        ? isProjectFolder
                          ? 'text-indigo-400'
                          : node.expanded ? 'text-amber-500' : 'text-amber-400 dark:text-amber-500'
                    ...
                    <span className="flex-1 min-w-0">
                      <span className="text-sm font-medium text-gray-800 dark:text-zinc-100 truncate block">
                        {node.entry.name}
                        {node.childLoading && (
                          <span className="ml-1.5 text-xs text-gray-400 dark:text-zinc-600">Loading…</span>
                        )}
                      </span>
                      {!isDir && (
                        <span className="flex items-center gap-1 mt-0.5">
                          {status?.analysed ? (
                            ...
                            <span className="text-[11px] text-gray-400 dark:text-zinc-600">Not yet analysed</span>
                          ) : (
                    ...
                    <span className="shrink-0 flex items-center gap-3" onClick={e => e.stopPropagation()}>
                      {!isDir && status && !status.analysed && !isBeingAnalysed && (
                        isAnalysable ? (
                          <button
                            ...
                            disabled={!!analysing}
                            className="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded border border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950 transition-colors disabled:opacity-40"
                          >
                            <Wand2 size={10} />
                            Analyse
                          </button>
                          ...
                          <Link
                            ...
                            className="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded border border-gray-200 dark:border-zinc-700 text-gray-400 dark:text-zinc-500 hover:text-indigo-500 hover:border-indigo-200 dark:hover:text-indigo-400 dark:hover:border-indigo-800 transition-colors"
                      ...
                      {!isDir && node.entry.size !== undefined && (
                        <span className="text-xs text-gray-400 dark:text-zinc-500">{formatBytes(node.entry.size)}</span>
                      )}
                      <span className="text-xs text-gray-400 dark:text-zinc-500">{formatDate(node.entry.modifiedAt)}</span>
                    </span>
                  </div>
                </div>
              )
