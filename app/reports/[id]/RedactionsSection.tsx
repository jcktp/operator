  ...
  return (
    <section>
      <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
        <EyeOff size={11} />
        Redactions
        <span className="ml-1 px-1.5 py-0.5 rounded bg-red-50 text-red-600 text-xs font-medium normal-case tracking-normal">
        ...
        {redactions.map((r, i) => (
          <div key={i} className="flex items-start gap-3 px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-gray-500 mb-1">{r.location}</p>
              <p className="text-sm text-gray-600 italic leading-relaxed">&ldquo;{r.context}&rdquo;</p>
            </div>
          </div>
        ))}
      </div>
