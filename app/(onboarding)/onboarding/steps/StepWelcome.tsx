import { Radio } from 'lucide-react'

interface Props {
  userName: string
  onNext: () => void
}

export default function StepWelcome({ userName, onNext }: Props) {
  return (
    <div className="space-y-8">
      <div className="text-center space-y-3">
        <div className="flex justify-center mb-2">
          <div className="w-12 h-12 bg-gray-900 dark:bg-zinc-100 rounded-2xl flex items-center justify-center">
            <Radio size={22} className="text-white dark:text-zinc-900" />
          </div>
        </div>
        <h1 className="text-2xl font-semibold text-gray-900 dark:text-zinc-50">
          Welcome to Operator{userName ? `, ${userName}` : ''}
        </h1>
        <p className="text-sm text-gray-500 dark:text-zinc-400">
          Let&apos;s get you set up in a couple of minutes.
        </p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl p-5">
          <p className="text-xs font-semibold text-gray-400 dark:text-zinc-500 uppercase tracking-wide mb-3">What it is</p>
          <ul className="space-y-2.5 text-sm text-gray-700 dark:text-zinc-300">
            <li>A local intelligence layer for your documents, notes, and sources</li>
            <li>All data stored on your machine — nothing sent to the cloud without your AI key</li>
            <li>An AI assistant that works with what you upload, not general knowledge</li>
          </ul>
        </div>
        <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl p-5">
          <p className="text-xs font-semibold text-gray-400 dark:text-zinc-500 uppercase tracking-wide mb-3">What it isn&apos;t</p>
          <ul className="space-y-2.5 text-sm text-gray-700 dark:text-zinc-300">
            <li>A search engine or general-purpose knowledge assistant</li>
            <li>A fact-checker — always verify AI outputs before acting on them</li>
            <li>A replacement for legal, financial, or medical professional judgment</li>
          </ul>
        </div>
      </div>

      <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-xl px-5 py-4 space-y-1.5">
        <p className="text-xs text-amber-800 dark:text-amber-300">
          <strong>AI outputs are probabilistic.</strong> They can be wrong, incomplete, or misleading. Always verify before acting on them.
        </p>
        <p className="text-xs text-amber-700 dark:text-amber-400">
          Open source under BSL 1.1 · free for personal and non-commercial use.
        </p>
      </div>

      <button
        onClick={onNext}
        className="w-full py-3 bg-gray-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-sm font-medium rounded-xl hover:bg-gray-700 dark:hover:bg-zinc-200 transition-colors"
      >
        Let&apos;s get you set up →
      </button>
    </div>
  )
}
